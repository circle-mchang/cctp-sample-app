...
import { getErrorMessage, WalletProviderError } from 'constants/errors'
...
export const testArbToAvaxFlow = () => {
  ...
  return {
    ...
    avaxChainParams,
    configValid:
      validateChainParameters(arbChainParams) &&
      validateChainParameters(avaxChainParams),
  }
}
  ...
  const switchNetwork = async (): Promise<SwitchNetworkResult> => {
    ...
    if (chainId !== null && chainId !== undefined) {
      ...
      if (chainId !== switchChainId) {
        ...
        // eslint-disable-next-line no-console
        console.log(
          `[useSwitchNetwork] Switching to chain ${chain} (${hexChainId})`
        )
        ...
        try {
          ...
          // eslint-disable-next-line no-console
          console.log(
            `[useSwitchNetwork] Switch failed with code ${switchError.code}:`,
            switchError.message
          )
          ...
          if (switchError.code === WalletProviderError.CHAIN_NOT_ADDED) {
            try {
              ...
              // eslint-disable-next-line no-console
              console.log(
                `[useSwitchNetwork] Adding chain ${chain} to wallet`,
                chainParams
              )
              ...
              if (!validateChainParameters(chainParams)) {
                ...
                // eslint-disable-next-line no-console
                console.error(
                  `[useSwitchNetwork] Invalid chain parameters for ${chain}`
                )
                setSwitchResult({
                  success: false,
                  error: errorMessage,
              ...
              // eslint-disable-next-line no-console
              console.log(
                `[useSwitchNetwork] Successfully added and switched to ${chain}`
              )
              setSwitchResult({ success: true, isLoading: false })
              return { success: true, isLoading: false }
            } catch (addError) {
              ...
              // eslint-disable-next-line no-console
              console.error(
                `[useSwitchNetwork] Failed to add ${chain}:`,
                addError
              )
              setSwitchResult({
                success: false,
            ...
            const errorMessage =
              getErrorMessage(switchError.code) ||
              `Failed to switch to ${chain}: ${switchError.message}`
            // eslint-disable-next-line no-console
            console.error(
              `[useSwitchNetwork] Failed to switch to ${chain}:`,
              switchError
            )
            setSwitchResult({
              success: false,
